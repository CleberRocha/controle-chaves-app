...
import { createChave, getLocais, getPerfis, getPessoas } from '../services/apiService';

// Componente para um item de permissão de pessoa na lista
...
export const NovaChave = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [locaisRes, perfisRes, pessoasRes] = await Promise.all([getLocais(), getPerfis(), getPessoas()]);
        setLocais(await locaisRes.json());
        setPerfis(await perfisRes.json());
  ...
  const handleConfirmPicker = (date) => {
    ...
    let formattedValue;
    if (mode === 'date') {
        // Formata para AAAA-MM-DD para o backend
        formattedValue = date.toISOString().split('T')[0];
    } else {
        // Formata para HH:MM para o backend
  ...
  const handleUpdatePessoaPermissao = (pessoaId, field, value) => {
    setPessoasPermitidas(prev => 
      prev.map(p => p.id === pessoaId ? { ...p, [field]: value } : p)
    );
  };

  const handleRemovePessoa = (pessoaId) => {
    setPessoasPermitidas(prev => prev.filter(p => p.id !== pessoaId));
  };
  
  const handleSalvar = async () => {
    if (!codigo.trim() || !local) {
      Alert.alert('Atenção', 'Código da chave e local são obrigatórios.');
      return;
    }
    setIsLoading(true);
    try {
      ...
      const response = await createChave(payload);
      if (!response.ok) throw new Error('Falha ao criar chave.');
      Alert.alert('Sucesso', 'Chave cadastrada!');
      navigation.goBack();
    } catch (error) {
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView>
        <View style={styles.mainContainer}>
          ...
          <TouchableOpacity onPress={() => setModal({visible: true, type: 'local'})} style={styles.inputContainer}><MapPin size={20} color="#6b7280" /><Text style={local ? styles.input : styles.placeholderText}>{local?.nome || "Selecione um local *"}</Text><ChevronDown size={20} color="#888" /></TouchableOpacity>
      ...
      <View style={styles.footerButtons}><TouchableOpacity onPress={handleSalvar} style={styles.saveButton} disabled={isLoading}>{isLoading ? <ActivityIndicator color="#fff" /> : <><Save size={20} color="#fff" /><Text style={styles.saveButtonText}>Salvar Cadastro</Text></>}</TouchableOpacity></View>

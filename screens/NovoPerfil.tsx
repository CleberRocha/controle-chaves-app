...
import { createPerfil } from '../services/apiService';
...
export const NovoPerfil = () => {
  ...
  const handleSalvar = async () => {
    ...
    setIsLoading(true);
    try {
      // CORREÇÃO APLICADA: Converte os dias (strings) para números antes de enviar
      ...
      if (response.status === 403) {
        Alert.alert('Acesso Negado', 'Você não tem permissão para criar perfis.');
        return;
      }
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.error || 'Falha ao criar perfil.');
      }
      Alert.alert('Sucesso', 'Perfil cadastrado!', [{ text: 'OK', onPress: () => navigation.goBack() }]);
    } catch (error) {
      setIsLoading(false);
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView>
        <View style={styles.mainContainer}>
          <Text style={styles.headerTitle}>Novo Perfil de Acesso</Text>
      ...
      <View style={styles.footer}>
        <TouchableOpacity onPress={handleSalvar} style={styles.saveButton} disabled={isLoading}>
          {isLoading ? <ActivityIndicator color="#fff" /> : <><Save size={20} color="#fff" /><Text style={styles.saveButtonText}>Salvar Perfil</Text></>}
        </TouchableOpacity>
      </View>
